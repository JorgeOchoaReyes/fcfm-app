...
export const mergeStates = (oldState: PriorityQueueStorage, incomingState: PriorityQueueStorage): PriorityQueueStorage => {
  ...
  incomingItems.forEach(remoteItem => {
    ...
    if (localItem) {
      // It exists locally, update it to the received update
      ...
      // Check if it's in our local history (already completed/deleted)
      const inLocalHistory = newState.history.some(h => h.code === remoteItem.code);
      if (!inLocalHistory) {
        // Add it
        if (remoteItem.status === "in-progress") {
          newState.inProgressItems.push(remoteItem);
  ...
  localItems.forEach(localItem => {
    const remoteHistoryItem = incomingState.history.find(h => h.code === localItem.code);
    const isStillActiveRemote = incomingItemMap.has(localItem.code);
    ...
    if (remoteHistoryItem && !isStillActiveRemote) {
      ...
      // Add to local history if not already there with same status
      const existingHistoryIndex = newState.history.findIndex(h => h.code === localItem.code && h.status === remoteHistoryItem.status);
  ...
  incomingState.history.forEach(remoteHItem => {
    const localHItem = newState.history.find(h => h.code === remoteHItem.code && h.status === remoteHItem.status);
    const isLocallyActive = newState.instanceTracker[remoteHItem.code];
    ...
    const unsubscribeNearby = Nearby.onTextReceived(async (event) => {
      try { 
        ...
        if (payload.type === "SYNC_STATE") {
          ...
          // Only merge if there's actually a change or remote is newer
          if (remoteState.lastUpdated >= localState.lastUpdated || JSON.stringify(remoteState.lastAction) !== JSON.stringify(localState.lastAction)) {
